...
def index():
    ...
    return render_template_string(open('index.html').read(), threads=threads)
...
def create_thread():
    if request.method == 'POST':
        ...
        if not author:
            return render_template_string(open('create_thread.html').read(), error="Ник не может быть пустым!")
        if author.lower() == 'admin':
            return render_template_string(open('create_thread.html').read(), error="Ник 'Admin' нельзя использовать!")
    ...
    return render_template_string(open('create_thread.html').read())
...
def thread(thread_id):
    ...
    if request.method == 'POST':
        ...
        db.session.commit()
        return redirect(f'/thread/{thread.id}')  # Перезагружаем страницу с новыми комментариями

    return render_template_string(open('thread.html').read(), thread=thread, comments=comments)
...
def admin_login():
    ...
    return render_template_string(open('admin_login.html').read())

# Страница админ-панели
@app.route('/admin/dashboard')
def admin_dashboard():
    if 'admin_logged_in' not in session or not session['admin_logged_in']:
        return redirect('/admin')  # Если не авторизован, перенаправляем на страницу логина

    threads = Thread.query.all()
    comments = Comment.query.all()  # Получаем все комментарии, чтобы отображать их в админке
    return render_template_string(open('admin_dashboard.html').read(), threads=threads, comments=comments)
...
def create_comment(thread_id):
    ...
    if author.lower() == 'admin':
        return redirect(f'/thread/{thread_id}')  # Перенаправляем на тред без создания комментария

    comment = Comment(content=content, author=author, created_at=datetime.now(), thread_id=thread_id)
    db.session.add(comment)
    db.session.commit()
    return redirect(f'/thread/{thread_id}')  # Перезагружаем страницу с новыми комментариями
...
def delete_comment(comment_id):
    ...
    db.session.commit()
    return redirect('/admin/dashboard')  # Перенаправляем обратно на панель админа
